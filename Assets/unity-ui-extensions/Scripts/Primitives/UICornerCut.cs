...
//        [HideUnless("useColorDown")]
        ...
        protected override void OnPopulateMesh(Mesh toFill)
        {
            ...
            if (any && cornerSize.sqrMagnitude > 0)
            {
                ...
                if (makeColumns)
                {
                    ...
                    if (left)
                        AddSquare(
                            ...
                            rect, useColorUp ? colorUp : color32, vbo);
                    if (right)
                        AddSquare(
                            ur, lr,
                            new Vector2(rectNew.xMax, rect.yMin),
                            new Vector2(rectNew.xMax, rect.yMax),
                            rect, useColorDown ? colorDown : color32, vbo);
                }
                else
                {
                ...
                //center
                if (makeColumns)
                    AddSquare(new Rect(rectNew.xMin, rect.yMin, rectNew.width, rect.height), rect, color32, vbo);
                else
                    AddSquare(new Rect(rect.xMin, rectNew.yMin, rect.width, rectNew.height), rect, color32, vbo);
